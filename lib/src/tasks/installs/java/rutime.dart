...
class Runtime {
  static Future<void> installJvmRuntime(String jvmVersion,
      String minecraftDirectory, InstallModel installModel) async {
    installModel.setState("Installing Java");

    if (getExecutablePath(jvmVersion, minecraftDirectory) != null) return;

    final String jvmManifestUrl =
    ...
    final basePath = p.join(
        minecraftDirectory, 'runtime', jvmVersion, platformString, jvmVersion);
    ...
    for (var key in platformManifest['files'].keys) {
      ...
      installModel.setProgress(
          ((count / platformManifest['files'].keys.length) * 100)
              .roundToDouble());
    }

    final versionPath = p.join(
        minecraftDirectory, 'runtime', jvmVersion, platformString, '.version');
    final versionFile = File(versionPath);
    await versionFile.writeAsString(
        manifestData[platformString][jvmVersion][0]['version']['name']);
  }
  ...
  static String getJvmPlatformString() {
    ...
    if (Platform.isWindows) {
      ...
      if (arch == "ARM64") {
        //No 32bit support needed, cause who uses a 32bit old mac
        return "mac-os-arm64";
      } else {
        return "mac-os";
      }
  ...
  static String? getExecutablePath(
      String jvmVersion, String minecraftDirectory) {
    String jvmPlatformString = getJvmPlatformString();

    String javaPath = p.join(minecraftDirectory, "runtime", jvmVersion,
        jvmPlatformString, jvmVersion, "bin", "java");
    ...
    javaPath = javaPath.replaceAll(p.join("bin", "java"),
