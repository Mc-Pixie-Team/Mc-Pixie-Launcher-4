...
class RunProcessors {
  static Future<void> runProcessors(
    ...
    installModel.setState("Running Forge Processors");
    String path = minecraftDirectory;

    Map<String, String> argumentVars = {
      "{MINECRAFT_JAR}": p.join(
          path, "versions", data["minecraft"], "${data["minecraft"]}.jar")
    };

    data["data"].forEach((dataKey, dataValue) {
      if (dataValue["client"].startsWith("[") &&
          dataValue["client"].endsWith("]")) {
        argumentVars["{$dataKey}"] = getLibraryPath(
            dataValue["client"].substring(1, dataValue["client"].length - 1),
            path);
      } else {
    ...
    for (int count = 0; count < data["processors"].length; count++) {
      ...
      if (!i.containsKey("sides") || i["sides"].contains("client")) {
        ...
        String mainClass =
            await getJarMainClass(getLibraryPath(i["jar"], path));
        ...
        for (var c in i["args"]) {
          ...
          if (vari.startsWith("[") && vari.endsWith("]")) {
            command
                .add(getLibraryPath(vari.substring(1, vari.length - 1), path));
          } else {
            command.add(vari);
          }
        }
        ...
        result.stdout.listen((i) {
          log.addAll(i);
        });
        var logfilePath =
            p.join(pixieInstances(), "logs", "processors", '$count.log');
        ...
        installModel.setProgress(
            ((count / (data["processors"].length - 1)) * 100).roundToDouble());
      }
    }
  }
  ...
  static Future<String> getJarMainClass(String jarPath) async {
    // Implement this function based on your requirements
    List<int> byte_MANIFEST = await InstallUtils.extractFilefromjar(jarPath,
