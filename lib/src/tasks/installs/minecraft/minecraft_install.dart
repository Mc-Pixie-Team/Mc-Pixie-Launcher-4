...
//MARK: INSTALL
  static Future install(
    ...
    await Installs.installLibraries(
        versiondata["libraries"], path, nativesPath, installModel);
    await Installs.installAssets(versiondata, path, installModel);

    if (versiondata["logging"] != null) {
      var logger_file = p.join(path, "assets", "log_configs",
          versiondata["logging"]["client"]["file"]["id"]);
      await Downloader(
              versiondata["logging"]["client"]["file"]["url"], logger_file)
          .startDownload();
    }
    ...
    if (versiondata["downloads"] != null) {
      ...
      await Downloader(
              ...
              p.join(path, "versions", versiondata["id"],
                  versiondata["id"] + ".jar"))
          .startDownload();
    }

    if (versiondata["javaVersion"] != null) {
      await Runtime.installJvmRuntime(
          versiondata["javaVersion"]["component"], path, installModel);
    }
...
//MARK: RUN
  static Future<Process> run(
      Version version, String processId, InstallModel installModel) async {
    ...
    var versiondata = jsonDecode(
        File(p.join(path, "versions", "$version", "$version.json"))
            .readAsStringSync());
