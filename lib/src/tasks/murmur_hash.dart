...
class murmur2 {
  ...
  static Future<Uint8List> get_jar_contents(String path) async {
    if (!File(path).existsSync())
      throw PathExistsException(path, OSError("File doesnt exist"));
    var result;
    try {
      result = await platform
          .invokeMethod<Uint8List>('get_jar_contents', {"path": path});
  ...
  static Future<int> compute_hash(Uint8List buffer) async {
    ...
    try {
      result =
          await platform.invokeMethod<int>('compute_hash', {"buffer": buffer});
    } on PlatformException catch (e) {
      print(e);
      result = 0;
    }
