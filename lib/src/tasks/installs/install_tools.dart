...
//MARK: InstallLibraries
  static Future<void> installLibraries(List<dynamic> libraries, String path,
      String nativesPath, InstallModel installModel) async {
    installModel.setState("Installing Libraries");
    ...
    for (int count = 0; count <= libraries.length; count += librariesStack) {
      installModel
          .setProgress(((count / libraries.length) * 100).roundToDouble());
      Iterable<Future<dynamic>> librariesInstallations = Iterable.generate(
          librariesStack > _total ? _total : librariesStack, (index) async {
        ...
        if (native != "") {
          await Downloader(
                  ...
                  p.join(currentPath,
                      i['downloads']['classifiers'][native]['path']))
              .startDownload();
          ...
          extractZipArchiveSync(
              File(p.join(
                  currentPath, i['downloads']['classifiers'][native]['path'])),
              Directory(nativesPath),
              exclude: i['extract'] == null ? null : excludeObj);
        }
      });
    ...
    installModel.setProgress(100);
  }

//MARK: InstallAssets

  static Future<void> installAssets(
      Map data, String path, InstallModel installModel) async {
    if (data["assetIndex"] == null) {
      return;
    }
    installModel.setState("Installing Assets");
    ...
    for (int count = 0; count <= values.length; count += assetsStack) {
      installModel.setProgress(
          ((count / (assetsData["objects"] as Map).values.length) * 100)
              .roundToDouble());

      Iterable<Future<dynamic>> assetsInstallation = Iterable.generate(
          assetsStack > _total ? _total : assetsStack, (index) async {
        var value = values[count + index];

        var downloadurl =
            'https://resources.download.minecraft.net/${value["hash"].substring(0, 2)}/${value["hash"]}';
        var savePath = p.join(assetsPath, 'objects',
            value["hash"].substring(0, 2), value["hash"]);

        if (File(savePath).existsSync()) return;

        try {
          await Downloader(downloadurl, savePath).startDownload();
        } catch (e) {
