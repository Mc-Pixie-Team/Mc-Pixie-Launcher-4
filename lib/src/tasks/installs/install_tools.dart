...
static Future<void> installLibraries(List<dynamic> libraries,
  String path, String nativesPath, InstallModel installModel) async {
  ...
  installModel.setProgress(((count / libraries.length) * 100).roundToDouble());
...
static Future<void> installAssets(Map data, String path, InstallModel installModel) async {
  ...
  for (int count = 0; count <= values.length; count += assetsStack) {
    installModel.setProgress(((count/ (assetsData["objects"] as Map).values.length) * 100).roundToDouble());
