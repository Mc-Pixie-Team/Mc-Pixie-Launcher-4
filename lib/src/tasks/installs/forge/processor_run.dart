...
class RunProcessors {
 static Future<void> runProcessors(Map<String, dynamic> data, String minecraftDirectory, String installerPath, String lzmaPath, String javaPath, InstallModel installModel) async {
  ...
  data["data"].forEach((dataKey, dataValue) {
    ...
    for (int count = 0; count < data["processors"].length; count++) {
      ...
      installModel.setProgress(((count / (data["processors"].length -1)) * 100).roundToDouble());
      }
    }
...
static String getClasspathSeparator() {
 ...
 static Future<String> getJarMainClass(String jarPath) async{
  // Implement this function based on your requirements
      List<int> byte_MANIFEST = await InstallUtils.extractFilefromjar(
